...
module.exports = new class commentsService {
    ...
    async createComment(postId, userId, message, isChanged = false, isPinned = false) {
        ...
        await postsModel.updateOne({_id: postId}, {$push: {comments: commentDTO.id}});
        return commentDTO;
    }
    ...
    async pinComment(id) {
        const pinnedComment = await commentsModel.updateOne({_id: id}, {isPinned: true});
        return pinnedComment.modifiedCount === 1;
    }

    async unPinComment(id) {
        const unPinnedComment = await commentsModel.updateOne({_id: id}, {isPinned: false});
